...
internal class B_CameraController : Behavior, IUpdatable
{
    ...
    // {
    //     if (InputManager.GetKeyDown(Key.C))
    //     {
    ...
    private void HandleMovement()
    {
        ...
        float ud = InputManager.GetAxis("UP_DOWN");
        ...
        if (InputManager.GetKey(Key.RightShift) || InputManager.GetKey(Key.LeftShift))
    ...
    {
        if (!InputManager.GetMouseButton(movementBtn))
            return;

        _yaw -= InputManager.MouseDelta.X * sensitivity;
        _pitch -= -InputManager.MouseDelta.Y * sensitivity;
    ...
    private void HandleSpeedScroll()
    {
        ...
        float scrollY = InputManager.MouseScrollDelta.Y;
    ...
    private void HandleZoom()
    {
        ...
        if (projecting == CameraProjectionType.Orthographic)
        {
            scrollY += InputManager.GetAxis("Vertical");
        }
        if (System.MathF.Abs(scrollY) < 0.01f)
            return;
    ...
    private void HandleProjectionSwitch()
    {
        ...
        bool switchedToPerspective = InputManager.GetKeyDown(Key.F1);
        bool switchedToOrtho = InputManager.GetKeyDown(Key.F2);
